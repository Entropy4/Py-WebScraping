...
def anystar_scraper(num):
    ...
    for i in range(1,51):
        res = requests.get(store_url.format(i))
        soup = bs4.BeautifulSoup(res.text, 'lxml')
        products = soup.select('.product_pod')
        for product in products:
            if len(  product.select( star_rating.format(num_keys[num]) )  ) != 0 :    
...
def driver_fn():
    print("\nBooks-to-Scrape Web Scraper v1.0")
